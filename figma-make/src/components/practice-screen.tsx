...
import { Sparkles, Loader2, BookMarked, LogOut, User as UserIcon, ChevronDown } from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { MacroCategory, Category, DsaCategory, FrontendCategory, SystemDesignCategory, Difficulty, Provider, ProblemPack } from '../types/problem';
...
interface PracticeScreenProps {
  onGenerate: (macroCategory: MacroCategory, category: Category, difficulty: Difficulty, provider: Provider, customPrompt?: string) => Promise<ProblemPack>;
...
export function PracticeScreen({ onGenerate, onNavigateSaved, user, onLogout, onSaveProblem }: PracticeScreenProps) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      const problem = await onGenerate(selectedMacroCategory, selectedCategory, selectedDifficulty, selectedProvider, customPrompt);
      setCurrentProblem(problem);
      
      // Scroll to workspace after a brief delay
      setTimeout(() => {
        workspaceRef.current?.scrollIntoView({ behavior: shouldReduceMotion ? 'auto' : 'smooth', block: 'start' });
      }, 300);
    } catch (error) {
        ...
        <div className="max-w-7xl mx-auto px-6 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              <Sparkles className="h-5 w-5" style={{ color: 'var(--accent-primary)' }} aria-hidden="true" />
              <h2>Improview</h2>
            </div>
          </div>
          ...
          {currentProblem && (
            <div className="flex justify-center pt-4">
              <motion.div
                ...
                style={{ color: 'var(--fg-muted)' }}
              >
                <span className="text-sm">Problem ready below</span>
                ...
                <div className="flex items-center gap-3">
                  ...
                  {onSaveProblem && (
                    <Button
                      variant="outline"
                      size="sm"
                      onClick={() => onSaveProblem(currentProblem)}
                      className="gap-2"
                    >
                      <BookMarked className="h-4 w-4" />
                      Save
